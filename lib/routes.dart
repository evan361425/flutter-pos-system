...
export 'package:possystem/routes/app_route_names.dart'
    show AppRouteNames, HomeMode, serializeRange;

import 'package:possystem/ui/analysis/analysis_view.dart';
import 'package:possystem/ui/analysis/history_page.dart';
import 'package:possystem/ui/analysis/manager_dashboard_screen.dart';
...
import 'package:possystem/ui/settings/cloud_sync_settings_screen.dart';
import 'package:possystem/ui/settings/printer_settings/printer_settings_screen.dart';
import 'package:possystem/ui/settings/staff_settings_screen.dart';
import 'package:possystem/ui/shift/close_register_screen.dart';
import 'package:possystem/ui/staff/lock_screen.dart';
...
import 'package:possystem/ui/tables/floor_plan_screen.dart';
...
class Routes {
  ...
  /// - Authenticated on lock → home (analysis)
  /// - Otherwise redirect to analysis if path is outside the base path
  static String? _redirect(BuildContext ctx, GoRouterState state) {
    final path = state.uri.path;
    final lockPath = AppRouteNames.path(AppRouteNames.lock);
    final isLock = path == lockPath || path.endsWith('/${AppRouteNames.lock}');
    final loggedIn = EmployeeManagerService.instance.isLoggedIn;
    ...
    return path.startsWith('$base/') ? null : '$base/anal';
  }

  /// Manager-only guard for Settings routes.
  static String? _redirectIfNotManager(BuildContext ctx, GoRouterState state) {
      ...
      GoRoute(
        ...
        routes: [
          StatefulShellRoute.indexedStack(
            ...
            // the order of this list should follow [homeBottomNavTabs]
            branches: [
              StatefulShellBranch(routes: [_analysisRoute]),
              StatefulShellBranch(routes: [_floorPlanRoute]),
      ...
      GoRoute(
        ...
        routes: [
          StatefulShellRoute.indexedStack(
            builder: (context, state, shell) =>
                HomePage(shell: shell, mode: homeMode),
            // Branch order must match [HomeTab] / [homeDrawerTabs] indices.
            branches: [
              StatefulShellBranch(routes: [_analysisRoute]),
              StatefulShellBranch(routes: [_stockRoute]),
              StatefulShellBranch(routes: [_cashierRoute]),
              ...
              if (!isProd)
                StatefulShellBranch(routes: [_debugRoute(inShell: true)]),
              StatefulShellBranch(
                routes: [
  ...
  static final _analysisRoute = GoRoute(
    ...
    routes: [
      GoRoute(
        name: AppRouteNames.managerDashboard,
        path: 'dashboard',
        parentNavigatorKey: rootNavigatorKey,
        redirect: _redirectIfNotManager,
        builder: (ctx, state) => _l(const ManagerDashboardScreen(), state),
      ),
      _createPrefixRoute(
        path: 'chart',
        prefix: 'anal',
  ...
  static final _stockRoute = GoRoute(
    ...
    routes: [
      ...
      GoRoute(
        ...
        parentNavigatorKey: rootNavigatorKey,
        pageBuilder: (ctx, state) =>
            MaterialDialogPage(child: _l(const ReplenishmentPage(), state)),
        routes: [
          GoRoute(
          ...
          GoRoute(
            ...
            redirect: _redirectIfMissed(
              path: 'stock/repl',
              hasItem: (id) => Replenisher.instance.hasItem(id),
            ),
            routes: [
              GoRoute(
                ...
                parentNavigatorKey: rootNavigatorKey,
                pageBuilder: (ctx, state) {
                  final repl = Replenisher.instance.getItem(
                    state.pathParameters['id']!,
                  )!;
                  return MaterialDialogPage(
              ...
              GoRoute(
                ...
                pageBuilder: (ctx, state) {
                  ...
                  return MaterialDialogPage(
                    child: _l(ReplenishmentPreviewPage(repl), state),
                  );
                },
              ),
            ],
  ...
  static final _cashierRoute = GoRoute(
    ...
    path: 'cashier',
    pageBuilder: (ctx, state) =>
        NoTransitionPage(child: _l(const CashierView(), state)),
    routes: [
      GoRoute(
        name: cashierChanger,
        path: 'changer',
      ...
      GoRoute(
        ...
        parentNavigatorKey: rootNavigatorKey,
        pageBuilder: (ctx, state) =>
            MaterialDialogPage(child: _l(const CashierSurplus(), state)),
      ),
    ],
  );
  static final _floorPlanRoute = GoRoute(
    name: AppRouteNames.floorPlan,
    path: 'tables',
    pageBuilder: (ctx, state) =>
        NoTransitionPage(child: _l(const FloorPlanScreen(), state)),
  );
  static GoRoute _orderAttrsRoute({required bool inShell}) => GoRoute(
    ...
    builder: (ctx, state) =>
        _w(_l(const OrderAttributePage(), state), S.orderAttributeTitle),
    routes: [
      GoRoute(
      ...
      GoRoute(
        ...
        parentNavigatorKey: rootNavigatorKey,
        pageBuilder: (ctx, state) =>
            MaterialDialogPage(child: _l(const OrderAttributeReorder(), state)),
      ),
      GoRoute(
        path: 'a/:id',
        parentNavigatorKey: rootNavigatorKey,
        ...
        routes: [
          ...
          GoRoute(
            ...
            parentNavigatorKey: rootNavigatorKey,
            pageBuilder: (ctx, state) {
              final oa = OrderAttributes.instance.getItem(
                state.pathParameters['id']!,
              )!;
              return MaterialDialogPage(
                child: _l(OrderAttributeOptionReorder(attribute: oa), state),
              );
            },
          ),
        ],
  ...
  static GoRoute _menuRoute({required bool inShell}) => GoRoute(
    ...
    builder: (ctx, state) {
      ...
      return _l(
        MenuPage(catalog: catalog, productOnly: mode == 'products'),
        state,
      );
    },
    routes: [
        ...
        routes: [
          GoRoute(
            ...
            pageBuilder: (ctx, state) {
              ...
              if (c == null) {
                return MaterialDialogPage(
                  child: _l(const CatalogModal(), state),
                );
              }
              return MaterialDialogPage(
                child: _l(ProductModal(catalog: c), state),
              );
            },
          ),
          GoRoute(
            ...
            routes: [
              ...
              GoRoute(
                ...
                pageBuilder: (ctx, state) {
                  ...
                  return MaterialDialogPage(
                    child: _l(ProductReorder(catalog), state),
                  );
                },
              ),
            ],
        ...
        routes: [
          GoRoute(
            ...
            pageBuilder: (ctx, state) {
              ...
              return MaterialDialogPage(
                child: _l(
                  ProductModal(product: product, catalog: product.catalog),
                  state,
                ),
              );
            },
          ...
          GoRoute(
            ...
            pageBuilder: (ctx, state) {
              ...
              if (ingr == null || qid == null) {
                return MaterialDialogPage(
                  child: _l(
                    ProductIngredientModal(product: p, ingredient: ingr),
                    state,
                  ),
                );
              }
              ...
              return MaterialDialogPage(
                child: _l(
                  ProductQuantityModal(quantity: qua, ingredient: ingr),
                  state,
                ),
              );
            },
          ...
          GoRoute(
            ...
            parentNavigatorKey: rootNavigatorKey,
            pageBuilder: (ctx, state) {
              final ingr = Menu.instance.getProduct(
                state.pathParameters['id']!,
              )!;
              return MaterialDialogPage(
                child: _l(ProductIngredientReorder(ingr), state),
              );
            },
          ),
        ],
  ...
  static GoRoute _printerRoute({required bool inShell}) => GoRoute(
    ...
    routes: [
      ...
      GoRoute(
        ...
        redirect: _redirectIfMissed(
          path: 'printer',
          hasItem: (id) => Printers.instance.hasItem(id),
        ),
        routes: [
          GoRoute(
            name: printerUpdate,
            ...
            pageBuilder: (ctx, state) {
              ...
              return MaterialDialogPage(
                child: _l(PrinterModal(printer: p), state),
              );
            },
          ),
        ],
  ...
  static GoRoute _quantitiesRoute({required bool inShell}) => GoRoute(
    ...
    parentNavigatorKey: inShell ? null : rootNavigatorKey,
    builder: (ctx, state) =>
        _w(_l(const QuantitiesPage(), state), S.stockQuantityTitle),
    routes: [
      GoRoute(
        name: quantityCreate,
        path: 'create',
        parentNavigatorKey: rootNavigatorKey,
        pageBuilder: (ctx, state) =>
            MaterialDialogPage(child: _l(const StockQuantityModal(), state)),
      ),
      GoRoute(
        path: 'a/:id',
        parentNavigatorKey: rootNavigatorKey,
  ...
  static GoRoute _settingsRoute({required bool inShell}) => GoRoute(
    ...
    routes: [
      GoRoute(
        name: settingsPrinterManagement,
        path: 'printer-management',
        parentNavigatorKey: rootNavigatorKey,
        builder: (ctx, state) => _l(const PrinterSettingsScreen(), state),
      ),
      GoRoute(
        name: AppRouteNames.settingsStaff,
        path: 'staff',
        parentNavigatorKey: rootNavigatorKey,
        builder: (ctx, state) => _l(const StaffSettingsScreen(), state),
      ),
      GoRoute(
        name: AppRouteNames.settingsCloudSync,
        path: 'cloud-sync',
        parentNavigatorKey: rootNavigatorKey,
        builder: (ctx, state) => _l(const CloudSyncSettingsScreen(), state),
      ),
      GoRoute(
        ...
        builder: (ctx, state) {
          ...
          final feature =
              Feature.values.firstWhereOrNull((e) => e.name == f) ?? .theme;
  ...
  static final _routes = [
    GoRoute(
      name: AppRouteNames.lock,
      path: 'lock',
      builder: (ctx, state) => _l(const LockScreen(), state),
    ),
    GoRoute(
      name: AppRouteNames.closeRegister,
      path: 'shift/close',
      builder: (ctx, state) => _l(const CloseRegisterScreen(), state),
    ),
    GoRoute(
      name: order,
      path: 'order',
  ...
  static const settingsPrinterManagement = 'settings.printerManagement';
